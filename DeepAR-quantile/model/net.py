...
class Net(nn.Module):
    ...
    def forward(self, x, alpha, idx, hidden, cell):
        '''
...
def quantile_loss_fn(y_pred: Variable, alpha: Variable, labels: Variable):
    '''
    Args:
        y_pred: (Variable) dimension [batch_size] - estimated mean at time step t
        alpha: (Variable) dimension [batch_size] - estimated standard deviation at time step t
        labels: (Variable) dimension [batch_size] z_t
    Returns:
        loss: (Variable) average log-likelihood loss across the batch
    '''
    ...
    loss_vector = torch.maximum(
      ...
      (alpha[zero_index] - 1) * (labels[zero_index] - y_pred[zero_index])
    )
